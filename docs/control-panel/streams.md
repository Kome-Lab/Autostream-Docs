...
Start ボタン、予約実行、API start に加えて、対象 Discord VC へのユーザー参加でも auto-start できます。対象になるのは、配信枠で `Discord VC参加で自動開始` をONにし、`created`、`draft`、`scheduled`、`ready` の待機状態にある stream だけです。Discord Bot は Control Panel の service-auth endpoint に開始を要求し、Control Panel 側で primary assignment、`streams.start` scope、保存済みの auto-start trigger を確認してから通常の start dispatch を行います。
