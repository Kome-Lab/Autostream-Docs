...
| Service heartbeat | 割り当て済みサービスが stale / offline ではないか | 対象サービスを起動または再起動します |
...
| Runtime config | 対象サービスが Control Panel 配布 config を読めるか | capability と service token scope を確認します |
...
| assigned service is stale | heartbeat が古い | 対象サービスの systemd/Docker 状態と network を確認します |
