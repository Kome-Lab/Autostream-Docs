...
Start ボタン、予約実行、API start に加えて、対象 Discord VC へのユーザー参加でも auto-start できます。Discord Bot は Control Panel の service-auth endpoint に開始を要求し、Control Panel 側で primary assignment と `streams.start` scope を確認してから通常の start dispatch を行います。
