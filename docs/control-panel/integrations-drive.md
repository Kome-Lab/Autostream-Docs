...
Integrations は、OAuth provider、OAuth connected account、Google Drive destination を管理する画面です。標準運用では、Google Drive upload の保存先は Streams の配信枠設定から作成され、Integrations では接続アカウントの状態確認を行います。
...
| Google Drive Destination | 保存先 folder、共有ドライブ、base path | Streams が作成した保存先の確認、Encoder Recorder |
...
login provider に Drive / YouTube scope を混ぜません。Google Drive upload や YouTube Live API は、配信用の OAuth Connected Account を Streams や YouTube Outputs で選びます。Google OAuth application 側には、`/auth/oauth/callback` と `/integrations/oauth-accounts/callback` の両方を redirect URI として登録します。Control Panel の OAuth Provider に保存する Redirect URI は `/auth/oauth/callback` だけです。
...
4. 用途を `YouTubeとDrive`、`YouTube Liveのみ`、`Archive保存のみ` から選びます。
5. `Connect with OAuth` を押します。
6. provider 側の認可画面で許可します。
7. Control Panel に戻り、Connected Account 一覧に出ることを確認します。
...
| OAuth account | 保存に使う connected account | StreamsのArchive OAuth accountから指定します |
...
5. 標準運用では、Streams の配信枠作成時に OAuth account、Folder ID、共有ドライブID、保存ファイル名を指定します。
6. Control Panel が内部の Drive Destination と Archive profile を生成します。
7. Integrations では生成済み destination の configured / masked 状態を確認します。

## Archive Settingsとの関係

Drive Destination を手作りしただけでは upload は有効になりません。標準運用では Streams の配信枠設定からDrive保存先を作ります。

1. Streams で配信枠を作成します。
2. Archive OAuth account と Drive Folder ID を入力します。
3. 共有ドライブを使う場合は共有ドライブIDも入力します。
4. 保存ファイル名を空欄にすると `配信枠名-年月日.mp4` になります。
5. 作成後、Integrations のDrive Destination一覧で configured / masked 状態を確認します。
...
| callback 後に account が増えない | Google OAuth application に `/integrations/oauth-accounts/callback` が登録されているか、Control Panel public URL、承認したscope |
| Drive folder が missing | Destination の Folder ID を再保存 |
| upload が forbidden | Drive folder の共有権限、OAuth account、Shared drive 設定 |
| Streams でArchive保存先を作れない | OAuth connected account、Drive scope、Folder ID、共有ドライブID |
