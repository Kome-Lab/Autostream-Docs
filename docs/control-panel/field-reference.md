...
Settings は、Control Panel 全体の表示名、時刻表示、アカウント通知に使うメールサーバー、ログイン時のBOT確認を管理する画面です。
...
| Cloudflare Turnstile | Site key と Secret key | login、メール変更承認のBOT確認 | secret は保存後に再表示されない |
