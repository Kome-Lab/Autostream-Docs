...
| Chat Channel ID | 配信画面へ載せる text channel の ID | chat表示を使う配信 | Worker events |
...
Watermark Settings は、配信映像に合成するロゴ画像と表示ルールを管理します。字幕やDiscord chatの描画は映像生成側の処理として扱います。

| 項目 | 入れるもの | 確認先 |
| --- | --- | --- |
| Name | profile 名 | Streams |
| 画像 | PNG / JPEG / WebP のウォーターマーク画像 | preview / 出力映像 |
| 表示位置 | watermark の表示位置 | preview / 出力映像 |
| 不透明度 | watermark の濃さ | preview / 出力映像 |
| 幅 | 画面幅に対する watermark の大きさ | preview / 出力映像 |

背景に対してウォーターマークが読みにくい場合は、位置、不透明度、幅を調整します。
