...
作成済みの通知先は一覧から編集できます。emailのRecipientsは空欄なら既存値を保持し、入力時だけ置き換えます。新規email通知先はSettingsの共通SMTPを使い、旧版の個別SMTP通知先は通常の編集では既存方式を保持します。SMTP未設定でもemail通知先は保存できますが、共通SMTP方式のテスト送信と実送信は `smtp_not_configured` で失敗します。
...
通知先を作成または編集したら、その channel の `テスト送信` を実行し、返された delivery result の `status` が `success` であることと、実際の通知先に届いたことを確認します。`failure` の場合は Notification Deliveries の安全化された error、Settingsの共通SMTP、通知経路の到達性を確認します。
