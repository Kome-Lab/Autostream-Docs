...
Settings は、Control Panel 全体の表示名、時刻表示、アカウント通知とemail通知先に共通利用するメールサーバー、ログイン時のBOT確認、Google Analyticsを管理する画面です。
...
| SMTP enabled | 共通SMTPを使うか | welcome email、email通知先を使うとき | Settings response の `smtp_password_configured` |
| SMTP Host / Port | メールサーバーの host と port | アカウント作成通知、email通知先 | Settingsのテスト送信 |
| SMTP From | 送信元メールアドレス | アカウント通知、email通知先 | 受信メールのFrom |
...
SMTP password は Control Panel の secret store に保存され、API response、Audit Logs、Docs、ブラウザ、Observability には raw value を出しません。テスト送信の response と Audit Logs には送信先を masked target として残します。remote SMTP では STARTTLS を有効にします。email通知はControl Panelの共通SMTP relayを使うため、Control PanelとSMTPサーバーの両方に到達できる必要があります。
...
| 録画プロファイル | 録画形式、保持日数、upload有無、Drive保存先をまとめた設定 | 録画する配信 | Archive / upload |
| YouTube Output | YouTube Outputs で作った配信先 | 外部配信するとき | YouTube readiness |

通常の配信枠作成画面はDiscord音声と生成映像を使うため、配信入力URLを表示しません。既存データと外部連携APIの互換性のため`encoder_input_url`は残っています。
...
Archive Settings は、録画形式、保存先、upload、保持期間をまとめた録画プロファイルを管理します。標準運用ではArchive画面でプロファイルを作り、Streamsの配信枠で選択します。

| 項目 | 入れるもの | いつ使うか | 保存後の確認 |
| --- | --- | --- | --- |
| Name | archive profile 名 | 録画設定を識別する場合 | Archive Settings一覧 |
| 録画形式 | MP4 / MKV | 録画する場合 | Archive / upload |
| Retention days | ローカル録画保持日数 | disk 容量管理 | disk free metric |
| Drive destination | Archive画面で作った保存先 | Driveへuploadする場合 | Destination が selected |
| Upload final archive | upload 有効化 | Drive へ保存する場合 | Archive / upload |

作成した録画プロファイルは Streams の配信枠作成画面で選びます。録画しない配信では`録画しない`のままにします。
...
| 項目 | 入れるもの | いつ使うか | 保存後の確認 |
| --- | --- | --- | --- |
| Name | 保存先名 | 録画プロファイルから識別する | Destination一覧 |
| OAuth account | 保存に使う接続アカウント | Drive保存先を作成する時 | Connected Account |
...
| Severity filter | incident、診断、復旧で通知する重要度 | system通知量の調整 | テスト送信 |
| Event type filter | incident、診断、復旧で通知するevent | incidentだけ通知など | Notification Deliveries |
| Webhook URL | Discord clientからコピーした標準・PTB・Canary URL、Slack/generic URL | webhook方式。編集時の空欄は保存済みURLを保持 | masked targetのみ。raw URLは非表示 |
...
email通知先は、Settingsのメールサーバーでテスト送信して実着信を確認してから、そのchannelの`テスト送信`を実行します。返されたdelivery resultの`status`が`success`であること、Notification Deliveriesに同じ結果が残ること、実際の通知先に届いたことまで確認します。`failure` の場合は Notification Deliveries の安全化された error、Settingsの共通SMTP、通知経路の到達性を確認します。
