...
| 予約・準備 | created / draft / scheduled / ready の配信枠数 | 待機中の配信枠を確認する | Streams |
| 要確認 | failed / error の配信数 | 異常があるか見る | Incidents |
| オンラインNode | online の Node 数 | 必要サービスが起動しているか見る | Service Health |
| 配信状態 | live、予約・準備、要確認の内訳 | 対象配信がある時だけ表示される | Streams |
| Metrics | Observability から届いた最新値 | metric が届いた時だけ表示される | Metrics |
| 本日の配信予定 | 当日予定または待機中の配信枠 | 次に動く枠を確認する | Streams |
| 最近の操作 | Audit Log の直近イベント | 直近の Start / Stop / 設定変更を確認する | Audit Logs |

Dashboard だけで設定変更はしません。異常を見つけたら、必ず専用画面で原因を確認します。

## Settings

Settings は、Control Panel 全体の表示名、時刻表示、アカウント通知に使うメールサーバーを管理する画面です。

| 項目 | 入れるもの | いつ使うか | 保存後の確認 |
| --- | --- | --- | --- |
| App name | 運用者が識別できる管理画面名 | 複数環境や複数顧客を扱うとき | サイドバー、ログイン、初期作成画面 |
| タイムゾーン | `Asia/Tokyo` などの IANA timezone | 番組表、監査ログ、配信予定を同じ基準で見るとき | Dashboard、Streams、Audit Logs、Account |
...
タイムゾーンを変えても service の起動時刻やログの保存時刻は変わりません。Control Panel の画面表示と本日の配信予定の判定に使います。
SMTP password は secret store に保存され、API response、Audit Logs、Docs には raw value を出しません。remote SMTP では STARTTLS を有効にします。
...
| 予定開始 | 配信を開始する予定日時 | Dashboard の本日の配信予定に出すとき | 本日の配信予定 |
| 予定終了 | 配信を終了する予定日時 | 終了予定を明示したいとき | 本日の配信予定 |
...
| Chat Channel ID | 配信画面へ載せる text channel の ID | chat overlay を使う配信 | Worker events |
...
| 言語 | 主な音声言語 | 字幕の内容 |
| プロバイダ | STT provider または手動字幕 | Worker events |
...
| Name | profile 名 | Streams |
| 表示位置 | safe area | 文字切れ |
| テーマ | overlay theme | 出力映像 |
| ウォーターマーク | 表示有無、文字、位置、不透明度 | 出力映像 |

背景に対して文字やウォーターマークが読みにくい場合は、overlay 側の theme、safe area、不透明度を調整します。

## Archive Settings

Archive Settings は、互換用の録画、保存先、upload、保持期間を管理します。標準運用ではStreamsの配信枠でArchive OAuth Account、Drive Folder ID、共有ドライブID、保存ファイル名を指定します。
...
| Bot service ID | Discord Bot の Node ID | runtime config を対象Botへ渡す | Service Health |
...
Google Drive や YouTube Live API を使う場合は、配信用のOAuth Connected Accountを作成し、そのaccountをYouTube OutputsやStreamsのArchive保存先で選びます。login providerにDrive/YouTube scopeを混ぜません。Google OAuth application側には、ログイン用の `/auth/oauth/callback` に加えて、連携アカウント用の `/integrations/oauth-accounts/callback` も redirect URI として登録します。ただしControl Panelに保存するProviderのRedirect URIは `/auth/oauth/callback` です。
...
| Email | 登録完了メールと本人連絡先 | welcome email、アカウント確認 | Users一覧 |
