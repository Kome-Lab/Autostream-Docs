...
| Watermark Settings | 配信映像へ載せる1920x1080固定のウォーターマーク画像 | Encoder Recorder |
...
Watermark Settings は、配信映像に合成する1920x1080固定のウォーターマーク画像を管理します。字幕やDiscord chatの描画は、この設定ではなく映像生成側の処理として扱います。

| 項目 | 意味 |
| --- | --- |
| 画像 | PNG / JPEG / WebP の1920x1080画像 |
| 合成サイズ | 1920x1080固定 |
| フィット | 出力解像度が1080未満の場合は自動縮小 |

watermark は配信画面に出るため、画像が切れていないか、背景と重なって読みにくくないかを preview と test stream で確認します。表示位置や余白はアップロードする1920x1080画像側で調整します。

## Archive Settings

Archive Settings は、録画形式、ローカル保持日数、Drive保存先、upload有無をまとめる録画プロファイルです。Streams の配信枠作成時は、ここで作成したプロファイルを名前で選びます。
...
| Drive destination | 録画後の保存先 | Drive uploadを使う場合に選ぶ |
| Upload final archive | upload を有効にするか | 本番保存するなら on |

## Archive Settings の作成手順

1. Driveへ保存する場合は、Archive画面で先にDrive保存先を作ります。
2. `Name`、録画形式、保持日数、Drive destination、upload有無を入力して録画プロファイルを作ります。
3. Streams で配信枠を作る時に`録画プロファイル`から選びます。
