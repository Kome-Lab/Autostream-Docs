...
作成済み設定の一覧には、APIで削除できる項目だけ `削除` ボタンが表示されます。対象は Encoder / Caption / Overlay / Archive profile、Discord BOT settings、YouTube Outputs、OAuth Providers、OAuth Connected Accounts、Drive Destinations、Users、Roles、Notification Channels です。
...
2. 既に作成済みの `Discord Config`、`YouTube Output`、`Encoder Profile` を選びます。
3. Google Driveへ保存する場合は、Archive OAuth account、Drive Folder ID、必要なら共有ドライブIDと保存ファイル名を入力します。
4. 必要なら `Caption Profile` と `Overlay Profile` を選びます。
5. `Create Stream With Current Settings` を押します。
6. `Stream` の選択が新しい配信に切り替わったことを確認します。
7. `Save Settings` を押し、選択した設定を保存します。
...
3. 変更したい profile、Discord、YouTube、Archive保存先を選び直します。
...
5. `Bot service ID` に online の Discord Bot の Node ID を入れます。
...
3. 必要ならウォーターマーク表示、文字、位置、不透明度を設定します。
4. Streams の `Overlay Profile` に紐づけます。
5. Worker event test を使い、Encoder Recorder 側に event が届くか確認します。
6. テスト配信で文字やウォーターマークが切れないか、背景と重なって読みにくくないか確認します。

暗い背景、明るい背景の両方で読めることを確認してください。

## Archive Settings

Archive Settings は、互換用の録画、変換、upload、保持期間を管理します。標準運用では、配信枠のStreams画面でArchive OAuth account、Drive Folder ID、共有ドライブID、保存ファイル名を設定します。

### 作成手順

1. Integrations でDrive用のOAuth Connected Accountを作ります。
2. Streamsで配信枠を作成します。
3. Archive OAuth account、Drive Folder ID、必要なら共有ドライブIDを入れます。
4. 保存ファイル名を空欄にすると `配信枠名-年月日.mp4` になります。
5. テスト配信で final MKV / MP4 が作られることを確認します。
6. 保存先の権限とupload結果を確認します。
...
2. 用途を `YouTubeとDrive`、`YouTube Liveのみ`、`Archive保存のみ` から選びます。
3. provider 側で権限を承認します。
4. Control Panel に戻ったら connected account が増えているか確認します。
5. YouTube Outputs や StreamsのArchive保存先でその account を選びます。
...
Settings は、Control Panel の表示名、時刻表示の基準、アカウント通知用SMTPを管理する画面です。
...
| メールサーバー | Users の登録完了メール | SMTP host、port、From、STARTTLS、認証情報を確認する |

SMTP password は保存後に再表示されません。SMTPを無効化すると、登録完了メールは送信されず、保存済み password も解除されます。
...
| 自分のログイン設定を変えたい | Account | メール、MFA、Passkey、OAuth連携 |

## Account

Account は、ログイン中のユーザー自身がメールアドレス、password、MFA、Passkey、OAuth連携を管理する画面です。

アカウントメールは、通知やアカウント登録完了メールの本人連絡先として使います。空で保存すると未設定に戻ります。OAuth連携の一覧と解除は self 用 API だけを使います。`users.read` や `users.manage_mfa` を持たない運用者でも、自分の連携状態だけを確認できます。

| 操作 | 確認すること |
| --- | --- |
| メール変更 | 通知や本人連絡先に使うメールアドレスだけが更新される |
| Password変更 | 成功後に全セッションが削除され、再ログインが必要になる |
| MFA | 有効状態、policy、recovery code の残数を確認する |
| Passkey | 登録済み端末を確認し、不要なcredentialを削除する |
