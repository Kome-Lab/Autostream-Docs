...
1. API Tokens でサービス登録用 token を作る。
...
| stale service | heartbeat が古い | 対象サービスの systemd / Docker / network を確認する |
...
3. Control Panel の API Tokens で Discord Bot 用 token を作ります。
4. Discord Bot service を起動し、Service Health で online を確認します。
5. Discord Settings を開きます。
6. `Bot service ID` に online の Discord Bot の `SERVICE_ID` を入れます。
...
| Health status | healthy / stale / no heartbeat | network、Control Panel URL、token を確認 |
...
Service Health で service が見えない場合は、API Tokens、`SERVICE_ID`、`CONTROL_PANEL_URL`、`CONTROL_PANEL_TOKEN` を確認します。
...
API Tokens は、各サービスが Control Panel に登録するための token を作る画面です。

1. service type を選びます。
2. 可能なら service ID を決めておきます。
3. 必要な scope だけを選びます。
4. expiration を設定します。
5. 作成時に一度だけ表示される token を対象サービスの env または secret store に入れます。
6. Service Health で登録されたことを確認します。

token は画面やドキュメント、チャット、GitHub に残さないでください。漏えいの疑いがある場合は rotate または revoke します。
...
| サービスが見えない | Service Health | API Tokens、service env |
