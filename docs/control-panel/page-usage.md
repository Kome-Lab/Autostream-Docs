...
3. ローカル保持日数を入力します。Google Driveへ保存する場合は、Archive OAuth account、Drive Folder ID、必要なら共有ドライブIDと保存ファイル名も入力します。
4. 必要なら `Caption Profile` を選び、ロゴを載せる配信だけ `ウォーターマークを使う` をONにして `Watermark Profile` を選びます。
...
Watermark Settings は、配信映像に合成するロゴ画像と表示ルールを決めます。字幕やDiscord chatの描画はこの画面ではなく映像生成側で扱います。

1. ウォーターマークの用途を決めます。番組ロゴ、局名、自治体名などです。
2. profile 名を付けて保存します。
3. PNG / JPEG / WebP の画像を選び、preview で位置、不透明度、幅を確認します。
4. Streams で `ウォーターマークを使う` をONにした配信だけ `Watermark Profile` に紐づけます。
5. テスト配信でウォーターマークが切れないか、背景と重なって読みにくくないか確認します。
