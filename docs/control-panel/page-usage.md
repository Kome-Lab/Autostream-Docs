...
6. YouTube OutputsにはYouTube対応、StreamsとDrive保存先にはDrive対応のaccountだけが表示されることを確認して選びます。両対応のaccountはどちらにも表示されます。
...
Settings は、Control Panel の表示名、時刻表示の基準、アカウント通知用SMTP、Cloudflare Turnstile、Google Analyticsを管理する画面です。
