...
| 順番 | 画面 | ここでやること |
| --- | --- | --- |
| 1 | Node登録 | 通常service用`config.yml`、または物理ホスト単位の`protocol major 2` Host AgentのAuto Configure commandを取得する |
...
| メニュー | 使う場面 | 次に読むページ |
| --- | --- | --- |
| Dashboard | いま配信中か、待機中の枠があるか、異常があるかをざっと見る | [ダッシュボード](/control-panel/dashboard) |
...
| Discord Settings | Discord Bot token、Discord BOT Node、音声転送、再接続を登録する | [DiscordとYouTube](/control-panel/discord-youtube) |
...
| Watermark Settings | 配信映像へ載せる1920x1080固定のウォーターマーク画像を設定する | [プロファイル設定](/control-panel/profiles) |
| Archive Settings | 録画、保存、Google Drive upload の設定を作る | [プロファイル設定](/control-panel/profiles) |
| Settings > Integrations | OAuth provider、YouTube・Drive接続、Drive destination を管理する | [OAuthとDrive保存先](/control-panel/integrations-drive) |
...
| Node登録 | 通常service用`config.yml`とtoken、endpointlessな`protocol major 2` Host Agentの4項目identityを作成する | [Node Agent登録](/control-panel/node-agent-registration) |
...
| Metrics | Grafana風の時系列グラフと最新値で metric を確認する | [監視と通知](/control-panel/observability) |
...
| 通知先 webhookと共通SMTPへの参照 | Notification Channels | incident通知の追加、停止、testを画面で行う。SMTP credentialはSettingsが所有 |
...
5. 字幕、チャット、参加者表示が怪しい場合は Worker events を確認します。

### 配信後

1. Streams で Stop を押します。
2. Archive / upload の status を確認します。
3. final MKV、final MP4、local artifact、upload result を確認します。
...
| Rotate / Revoke | Node Runtime Token や移行用 token を入れ替える、無効化する | rotate 後は `config.yml` や対象 env の更新が必要です |
...
| Bot が voice channel に入らない | Streams | Service Health、Incidents |
