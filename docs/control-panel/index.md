...
| 順番 | 画面 | ここでやること |
| --- | --- | --- |
| 1 | Node登録 | 通常service用`config.yml`、または中央Update Agent 1つのNode Runtime Tokenを取得する |
...
| Node登録 | 通常service用`config.yml`とtoken、中央Update Agent 1つのNode Runtime Tokenを作成する | [Node Agent登録](/control-panel/node-agent-registration) |
