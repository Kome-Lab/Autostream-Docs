...
4. 固定output relayではEncoderとYouTube Outputの両方で`live_api_relay_static`と同じ非secret binding IDを使います。keyをmodeやbindingへ入れません。詳細は[YouTube OutputsとDiscord設定](/control-panel/discord-youtube#固定relayのv2経路)を参照してください。
