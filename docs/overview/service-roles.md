...
6. 物理hostごとの非root Host AgentがControl Panelへoutbound HTTPSで接続し、root Local Executorへ固定Unix socketで依頼します。protocol major 2、policy、host ownership fenceが一致した更新だけを実行します。
...
| 独立Updater | host別policy、heartbeat、endpoint観測、Local Executor経由のservice更新、health確認、rollback | `protocol major 2` Host Agentは物理ホストごとに非rootで1つ。epoch `0`ではobserver、明示的切替後だけmutation |
...
- Update Agentは配信処理のNodeではありません。物理hostごとにendpointlessな独立Updaterを1つ置きます。Agentは非root・outbound HTTPSのみで、root operationはLocal Executorが固定socket、root policy、single-use grantで実行します。Control Panelやapplication serviceへDocker socketやroot権限を渡しません。
