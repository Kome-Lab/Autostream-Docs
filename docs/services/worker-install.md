...
| Stream ingest signing key | Control Panel と同じ `AUTOSTREAM_STREAM_INGEST_SIGNING_KEY` |
...
compose では Panel が生成した `config.yml` を read-only mount します。env には `AUTOSTREAM_NODE_CONFIG=/etc/autostream-worker/config.yml` と `AUTOSTREAM_STREAM_INGEST_SIGNING_KEY` を入れ、`CONTROL_PANEL_TOKEN` を手入力しません。
