...
| Encoder用service token | `CONTROL_PANEL_TOKEN` |
...
SERVICE_PUBLIC_URL=https://<ENCODER_SERVICE_HOST>
...
1. API Tokens で `encoder_recorder` 用 token を作ります。
2. Encoder Recorder を起動します。
3. Service Health で online と capability を確認します。
4. Encoder Profiles を作ります。
5. YouTube Outputs を作ります。
6. Drive destination と Archive Settings を作ります。
7. Streams で Encoder Profile、YouTube Output、Archive Profile を選びます。
8. Start前に preflight を確認します。
...
Docker image には FFmpeg を含める構成にできますが、release artifact のhost直接起動では含めません。compose では archive dir を volume として永続化し、output relay と同じ network で動かします。
