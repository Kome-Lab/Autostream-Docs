...
3. `config.yml` を `/etc/autostream-encoder-recorder/config.yml` に配置します。Node Runtime Token と `stream_ingest.signing_key` を含むため、ファイル権限は `0640` に制限します。
...
Docker image はDebianの`ffmpeg` packageを含み、HLS previewも同じEncoder Recorder API portを使います。preview用の追加公開portや追加volumeは不要です。composeでは既存archive dir volumeを`/var/lib/autostream/archives`へ永続化し、Panelが生成した`config.yml`を`/etc/autostream-encoder-recorder/config.yml`へread-only mountします。
