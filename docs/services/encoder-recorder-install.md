...
Docker image はDebianの`ffmpeg` packageを含み、HLS previewも同じEncoder Recorder API portを使います。preview用の追加公開portや追加volumeは不要です。composeではarchive dir volumeを`/var/lib/autostream/archives`へ永続化します。Configurationから手動保存した`config.yml`はread-only mountにします。Encoder Recorder repositoryのone-shot Auto Configureを使う場合だけ、生成時は`config` directoryを`/etc/autostream-encoder-recorder`へ書き込み可能でmountし、host側のdirectory権限を限定します。
