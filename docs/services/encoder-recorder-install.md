...
Node `config.yml`と`listener.credential: node-listener.json`の固定JSONがない場合や不正な場合はstartupがfail closedで終了します。先にAuto ConfigureでNode configを生成し、systemd `LoadCredential`で渡される待受address / revisionとNode identityを確認してからEncoder Recorderを起動してください。
...
`AUTOSTREAM_OUTPUT_RELAY_MODE`は必須です。`direct`ではrelay URLを設定せず、Control Panelのsecret referenceから配信先を解決します。固定relayでは`live_api_relay_static`、relay URL、同じ非secret binding IDを明示し、readyな同名YouTube Outputだけを使います。
...
binding IDはControl Panelの`relay_binding_id`と完全一致させます。key、RTMPS URL、視聴URLをbindingへ入れません。mode未設定、未知mode、URLとの矛盾、binding不一致はpreflight/startでfail closedです。

変更前に配信枠を停止し、設定後にService Health、readiness、小さな開始・停止を確認します。開始結果が不明なら固定Relay recoveryを先に完了してください。rollbackはreleaseと設定全体を復元します。keyの自動変換、複製、互換modeへの切戻しは行いません。
...
4. 必須Node configと`listener.credential: node-listener.json`の固定JSONを揃えてEncoder Recorderを起動します。systemd `LoadCredential`でJSONを渡し、欠落やservice type / revision不一致ではstartupを拒否します。設定更新後は`sudo systemctl restart autostream-encoder-recorder`を実行します。
