...
`AUTOSTREAM_STREAM_INGEST_SIGNING_KEY` は Control Panel と同じ値を入れます。Encoder Recorder の Observability signal は、標準構成では Node Runtime Token で Control Panel に送り、Control Panel が Observability へ転送します。Encoder Recorder env に `OBSERVABILITY_TOKEN` は入れません。直接ingest互換fallbackを使う場合だけ、`OBSERVABILITY_URL` と `OBSERVABILITY_TOKEN=<OBSERVABILITY_INGEST_TOKEN>` を追加します。生成方法は [秘密情報とtoken生成](/security/tokens) を参照してください。
