...
`AUTOSTREAM_STREAM_INGEST_SIGNING_KEY` は Control Panel と同じ値を入れます。Encoder Recorder の Observability signal は、Node Runtime Token で Control Panel に送り、Control Panel が Observability へ転送します。Encoder Recorder env に Observability 接続用tokenは入れません。生成方法は [秘密情報とtoken生成](/security/tokens) を参照してください。
...
3. `config.yml` を `/etc/autostream-encoder-recorder/config.yml` に配置します。
...
8. Google Driveへ保存する場合は、Drive用のOAuth Connected Accountを作ります。
9. Streams で Encoder Profile、YouTube Output、Archive OAuth account、Drive Folder ID、必要なら共有ドライブIDと保存ファイル名を選びます。
10. Start前に preflight を確認します。

## Google Drive保存

Google Drive へ保存する場合は、Control Panel の Streams で配信枠ごとに保存先を指定します。
...
共有ドライブを使う場合は、Streams の配信枠設定で共有ドライブを有効にし、共有ドライブIDを入力します。対象folderへ接続済みOAuth accountを参加させます。
...
Docker image には FFmpeg を含める構成にできますが、release artifact のhost直接起動では含めません。compose では archive dir を volume として永続化し、Panelが生成した `config.yml` を `/etc/autostream-encoder-recorder/config.yml` へ read-only mount します。
