...
`AUTOSTREAM_STREAM_INGEST_SIGNING_KEY` は Control Panel と同じ値を入れます。Encoder Recorder の `OBSERVABILITY_TOKEN` は signal 送信用の ingest token で、Control Panel の `OBSERVABILITY_TOKEN` に入れる admin token とは別です。生成方法は [秘密情報とtoken生成](/security/tokens) を参照してください。
