...
| `OBSERVABILITY_ADMIN_TOKEN_SHA256` | Control Panel 用 admin token hash |
| `OBSERVABILITY_ADMIN_TOKEN_BINDINGS` | admin token hash と scope の対応。標準構成では `observability.ingest` も含めます |
| `REMEDIATION_MODE` | 自動対応の扱い |

標準構成では、Worker / Encoder Recorder の signal は Control Panel 経由で届きます。Observability env には token の生値を置かず、Control Panel が使う admin token の SHA-256 と binding だけを置きます。直接ingest互換fallbackを使う場合だけ、`OBSERVABILITY_INGEST_TOKEN_SHA256` と `OBSERVABILITY_INGEST_TOKEN_BINDINGS` を追加します。Control Panel、Worker、Encoder Recorder 側の env との対応は [秘密情報とtoken生成](/security/tokens) を参照してください。
...
3. Worker や Encoder Recorder から Control Panel 経由で signal が届いているか確認します。
4. テスト通知を送ります。
5. incident が作成、解決できるか確認します。

通知が来ない場合は、通知先の登録、admin token binding、Control Panel から Observability へのネットワークを順番に確認してください。
