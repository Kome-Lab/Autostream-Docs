...
2. Discord Bot の env に `SERVICE_ID`、`CONTROL_PANEL_URL`、登録用 token を入れます。
...
2. Worker の env に `SERVICE_ID`、`CONTROL_PANEL_URL`、登録用 token を入れます。
3. Worker を起動します。
...
| event送信失敗 | Encoder Recorder URL、firewall、service token |
| Workerが表示されない | API token、SERVICE_ID、CONTROL_PANEL_URL |
...
3. Encoder Recorder の env に `SERVICE_ID`、`CONTROL_PANEL_URL`、登録用 token、archive directory などを入れます。
...
2. Observability の env に `SERVICE_ID`、`CONTROL_PANEL_URL`、登録用 token、database を入れます。
...
2. 対象サービスの env に token、`SERVICE_ID`、`CONTROL_PANEL_URL` を入れます。
3. サービスを起動します。
