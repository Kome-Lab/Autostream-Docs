...
| admin token hash | Control Panel から通知・incidentを管理し、signalを転送する |
...
標準構成では、Worker / Encoder Recorder の signal は Control Panel が Observability へ転送します。Observability env には `OBSERVABILITY_ADMIN_TOKEN` の生値を入れず、SHA-256 と binding だけを置きます。Worker / Encoder Recorder から直接ingestする互換fallbackを使う場合だけ、別途 ingest token hash と binding を追加します。生成方法と各サービスの対応は [秘密情報とtoken生成](/security/tokens) を参照してください。
...
admin token は、Control Panel が incident、notification channel、Monitoring 情報、signal 転送を扱うためのscopeを絞るために使います。Control Panel 経由の signal 転送には `observability.ingest` が必要です。

```text
OBSERVABILITY_ADMIN_TOKEN_BINDINGS=<SHA256_OF_OBSERVABILITY_ADMIN_TOKEN>:observability.read|observability.ingest|incidents.update|notifications.read|notifications.manage|remediation.read|remediation.approve|remediation.execute
```

Control Panel の env では、この admin token の生値を `OBSERVABILITY_TOKEN=<OBSERVABILITY_ADMIN_TOKEN>` として設定します。placeholder のままでは動きません。実際のhashは本番のsecret管理手順で生成してください。

直接ingest互換fallbackを使う場合だけ、ingest token はどのサービスがsignalを送ってよいかを絞るために使います。
...
直接ingest binding の service ID は Node登録で決めた Node ID と一致させます。Encoder Recorder の Node ID が `studio-encoder-01` なら、binding も `encoder_recorder:studio-encoder-01` にします。

## Control Panelで使う

1. Control Panel の env に `OBSERVABILITY_URL` と `OBSERVABILITY_TOKEN=<OBSERVABILITY_ADMIN_TOKEN>` を設定します。
...
| signalが届かない | Control Panel の `OBSERVABILITY_TOKEN`、admin binding の `observability.ingest` scope、Worker / Encoder Recorder の Node Runtime Token |
