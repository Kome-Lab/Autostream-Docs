...
| ingest token hash | Worker / Encoder Recorder からsignalを受ける |
...
`OBSERVABILITY_INGEST_TOKEN` と `OBSERVABILITY_ADMIN_TOKEN` の生値は、Observability の env には入れません。Observability 側には SHA-256 だけを置きます。生成方法と Control Panel / Worker / Encoder Recorder との対応は [秘密情報とtoken生成](/security/tokens) を参照してください。
...
binding の service ID は Node登録で決めた Node ID と一致させます。Encoder Recorder の Node ID が `studio-encoder-01` なら、binding も `encoder_recorder:studio-encoder-01` にします。
