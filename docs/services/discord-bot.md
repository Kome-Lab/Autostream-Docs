...
| `DISCORD_WORKER_EVENT_TIMEOUT_SEC` | Worker event 送信のtimeout。既定は `3` |
| `DISCORD_RECONNECT_*` | voice reconnect の基本設定 |
| `CONTROL_PANEL_RUNTIME_CONFIG_REFRESH_INTERVAL` | 待機枠やchannel設定を再読込する間隔。既定は `30s` |

Worker event の送信先URLとtokenは env に固定しません。配信開始時に Control Panel が、その配信枠で primary assigned された Worker を解決し、Discord Bot job の `worker_events_url` / `worker_events_token` として渡します。
...
Guild ID、VC Channel ID、Chat Channel ID は Streams の配信枠に保存します。VC参加で自動開始する場合は、同じ配信枠で `Discord VC参加で自動開始` をONにします。Chat Channel ID を入れると、配信開始後にその channel の新規messageを配信枠で選択された Worker の `overlay.discord_chat` に流します。
...
| `discord.worker_event_publish_failures_total` | 0 | Worker assignment、Worker到達性、署名鍵を確認 |
