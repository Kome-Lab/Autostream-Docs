...
8. 各targetの`/health`と`/version`を、そのtargetと同じhostのloopbackから取得できる。
...
top-levelと全targetの`host_id`は一致させます。`arch`はhelper artifactと実hostに一致させます。`health_url`と`version_url`は同じhostのloopbackだけを指定します。Control PanelとObservabilityではroot所有`backup_argv`が必須です。
...
helperを置かずに手動更新を続けることもできます。その場合Application Infoは検出専用です。手動更新後はbinaryの`--version`、systemd MainPIDまたはcontainer image ID、`/health`、`/version`、Service Health、短いテスト配信まで確認してください。
