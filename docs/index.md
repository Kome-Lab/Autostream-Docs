...
<div class="doc-grid">
  ...
  <a class="doc-card" href="/services/host-operations"><strong>6. サービスを導入する</strong><span>Control Panel、Bot、Worker、Encoder、ObservabilityをLinuxサーバーで動かします。</span></a>
...
- Control Panel の各画面を使う: [Control Panel画面の全体像](/control-panel/)
- Linuxサーバーでサービスを起動する: [サービス共通の導入と運用](/services/host-operations)
- Control Panelを本番向けに入れる: [Control Panelを導入する](/services/control-panel-install)
- Bot / Worker / Encoder / Observabilityを入れる: [サービス導入・運用](/services/host-operations)
- 配信を作成して開始する: [配信画面](/control-panel/streams)
- Discord と YouTube を設定する: [DiscordとYouTube](/control-panel/discord-youtube)
...
- サービスを割り当てる: [サービス割り当て](/control-panel/services-workers)
